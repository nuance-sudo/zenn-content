...
emoji: "🧠"
type: "tech"
topics: ["初心者", "agent", "memorybank", "googlecloud", "vertexai"]
published: true
---
...
[Vertex AI Agent Engine Memory Bank](https://docs.cloud.google.com/agent-builder/agent-engine/memory-bank/overview?hl=ja) は、ユーザーとエージェントの会話（セッション）に基づいて、**長期記憶を動的に生成・管理できるマネージドサービス**です。ステートレスなエージェントに特定のユーザーに紐づく記憶を持たせることで、セッションをまたいだパーソナライズされた体験を提供できます。
...
`Retrieve` は指定したスコープに完全一致するメモリのみを返し、`List` は Agent Engine 内の全メモリを返します。

![Retrieve：スコープ内の記憶を取得（user_123 の 8 件のみ）](/images/hello-memorybank/retrieve-scoped.png)
*Retrieve：scope に一致する user_123 のメモリのみが取得される*

![Get：リソース名で1件取得](/images/hello-memorybank/retrieve-get-one.png)
*Get：リソース名を指定して特定の記憶をピンポイントで参照*

![List：Agent Engine 内の全メモリを一覧取得（user_999 を含む 10 件）](/images/hello-memorybank/retrieve-list-all.png)
*List：user_999 のメモリも含め、スコープに関係なく全件が返っている*

### ② スコープの「完全一致」制約とメタデータによる検索設計

`Retrieve` を使う際、最初に意識すべき仕様が**スコープの指定方法と検索範囲**です。

記憶を取得する際、指定するスコープは**完全一致**でなければならないという制約があります。これを意識せずに記憶を保存していくと、後から横断的に検索する際に苦労することになります。
...
「基本はプロジェクトごとに分けたいが、一括で横断検索したいケースもある」という場合は、細分化の条件をスコープではなく**メタデータ**に持たせるのがよさそうです。
