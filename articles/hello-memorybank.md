...
[Vertex AI Agent Engine Memory Bank](https://cloud.google.com/agent-builder/docs/memory-bank?hl=ja) は、ユーザーとエージェントの会話（セッション）に基づいて、**長期記憶を動的に生成・管理できるマネージドサービス**です。ステートレスなエージェントに特定のユーザーに紐づく記憶を持たせることで、セッションをまたいだパーソナライズされた体験を提供できます。
